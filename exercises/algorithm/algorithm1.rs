    ...
    pub fn merge(mut list_a: LinkedList<T>, mut list_b: LinkedList<T>) -> Self {
        ...
        while a < list_a.length as i32 && b < list_b.length as i32 {
            if list_a.get(a).unwrap() <= list_b.get(b).unwrap() {
                list.add(list_a.get(a).unwrap().clone());
                a += 1;
            } else {
                list.add(list_b.get(b).unwrap().clone());
                b += 1;
            }
        }
...
impl<T: Clone + PartialOrd> Display for LinkedList<T>
where
    T: Display,
{
...
impl<T: Clone + PartialOrd> Display for Node<T>
where
    T: Display,
{
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
